          ...
          <div class="widget-content nopadding">
            <form enctype='multipart/form-data' class="form-horizontal" method="post" action="{{ url('admin/edit-banner/'.$bannerDetails->id)}}" name="edit_banner" id="edit_banner" novalidate="novalidate"> {{ csrf_field()}}
               ...
               <div class="control-group">
                ...
                <div class="controls">
                  <div class="uploader" id="uniform-undefined"> <input type="file" name="image" id="image" size="9" style="opacity: 0;"><span class="filename">No File Selected</span><span class="action">Choose File</span>
                  </div>
                    @if(!empty($bannerDetails->image)){{-- //if we don't want to add new iamge then we will pick current image --}}
                  <input type="hidden" name="current_image" value="{{ $bannerDetails->image}}">{{-- //hidden input type to display current image name --}}
                  @endif
